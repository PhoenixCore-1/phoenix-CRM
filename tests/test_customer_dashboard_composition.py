"""Tests for Phase 10.5 CRM dashboard composition."""
...
def test_composes_kpis_and_work_queue_into_one_read_model():
    ...
    result = CustomerDashboardCompositionService.build(
        tenant_id=tenant_id,
        user_id=uuid4(),
        kpis=_kpis(tenant_id),
        work_queue=queue,
...
def test_work_queue_section_has_deterministic_summary_metrics():
    ...
    assert metrics == {
        "total_work_items": 2,
        "due_or_overdue_work_items": 1,
        "overdue_work_items": 1,
        "call_work_items": 1,
        "follow_up_work_items": 1,
    }
...
def test_kpi_tenant_mismatch_is_rejected():
    ...
    with pytest.raises(ValueError, match="tenant"):
        CustomerDashboardCompositionService.build(
            tenant_id=tenant_id,
            user_id=uuid4(),
            kpis=_kpis(uuid4()),
            reference_at=datetime(2026, 9, 5, tzinfo=timezone.utc),
