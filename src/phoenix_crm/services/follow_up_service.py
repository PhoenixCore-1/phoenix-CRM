"""Follow-up application services for Phoenix CRM 360."""

from __future__ import annotations

from datetime import datetime
from uuid import UUID

from phoenix_crm.domain import Customer, CustomerFollowUp, FollowUpStatus


class FollowUpService:
    """Manage follow-up lifecycle and tenant/customer-scoped retrieval."""

    @staticmethod
    def record_follow_up(
        follow_up: CustomerFollowUp,
        customer: Customer,
    ) -> CustomerFollowUp:
        """Validate and record a follow-up against its CRM customer."""
        FollowUpService._validate_customer(follow_up, customer)
        return follow_up

    @staticmethod
    def complete(
        follow_up: CustomerFollowUp,
        customer: Customer,
        *,
        completed_at: datetime | None = None,
    ) -> CustomerFollowUp:
        """Complete a follow-up after validating its customer relationship."""
        FollowUpService._validate_customer(follow_up, customer)
        follow_up.complete(completed_at=completed_at)
        return follow_up

    @staticmethod
    def cancel(
        follow_up: CustomerFollowUp,
        customer: Customer,
    ) -> CustomerFollowUp:
        """Cancel a follow-up after validating its customer relationship."""
        FollowUpService._validate_customer(follow_up, customer)
        follow_up.cancel()
        return follow_up

    @staticmethod
    def reschedule(
        follow_up: CustomerFollowUp,
        customer: Customer,
        due_at: datetime,
    ) -> CustomerFollowUp:
        """Reschedule a follow-up after validating its customer relationship."""
        FollowUpService._validate_customer(follow_up, customer)
        follow_up.reschedule(due_at)
        return follow_up

    @staticmethod
    def mark_due(
        follow_up: CustomerFollowUp,
        customer: Customer,
    ) -> CustomerFollowUp:
        """Mark an active follow-up as due."""
        FollowUpService._validate_customer(follow_up, customer)
        follow_up.mark_due()
        return follow_up

    @staticmethod
    def for_customer(
        customer_id: UUID,
        follow_ups: list[CustomerFollowUp],
        *,
        tenant_id: UUID | None = None,
    ) -> tuple[CustomerFollowUp, ...]:
        """Return a customer's follow-ups, newest due date first."""
        items = [follow_up for follow_up in follow_ups if follow_up.customer_id == customer_id]
        if tenant_id is not None:
            items = [follow_up for follow_up in items if follow_up.tenant_id == tenant_id]
        items.sort(key=lambda item: (item.due_at, str(item.id)), reverse=True)
        return tuple(items)

    @staticmethod
    def assigned_to_user(
        user_id: UUID,
        follow_ups: list[CustomerFollowUp],
        *,
        tenant_id: UUID | None = None,
        include_completed: bool = True,
    ) -> tuple[CustomerFollowUp, ...]:
        """Return follow-ups assigned to a user, ordered by due date."""
        items = [item for item in follow_ups if item.assigned_to_user_id == user_id]
        if tenant_id is not None:
            items = [item for item in items if item.tenant_id == tenant_id]
        if not include_completed:
            items = [
                item
                for item in items
                if item.status is not FollowUpStatus.COMPLETED
            ]
        items.sort(key=lambda item: (item.due_at, str(item.id)))
        return tuple(items)

    @staticmethod
    def _validate_customer(follow_up: CustomerFollowUp, customer: Customer) -> None:
        if follow_up.tenant_id != customer.tenant_id:
            raise ValueError("Follow-up and customer must belong to the same tenant")
        if follow_up.customer_id != customer.id:
            raise ValueError("Follow-up customer does not match the supplied customer")
